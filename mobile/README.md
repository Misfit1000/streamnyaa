...
The v0.6 interface follows the desktop brand with a vanta-black base, restrained blood-red emphasis, translucent tonal surfaces, consistent 8/12 px geometry, and media-first layouts sized for narrow Android phones. Home uses a full-bleed swipeable spotlight that advances automatically, readable cinematic overlays, direct resume, and the desktop shelf order. Opening an anime enters one compact Android cinema screen and automatically prepares the best compatible source. Episode metadata and playback stay prominent; source modes, quality, audio, trust filters, sorting, sharing, and manual selection live in a progressive-disclosure sheet instead of turning the screen into a desktop control panel. Android system navigation starts hidden and can be revealed temporarily with an edge swipe.
