...
The v0.8 interface uses the supplied StreamNyaa cat mark and a watch-first Android layout: branded top bar, functional feed filters, rotating featured playback, My List, Continue Watching, a compact trending feed, and four labeled native tabs. Opening an anime silently prepares the best mobile-compatible release and keeps playback inside the app. Torrent-file metadata, DHT, local discovery, tracker reinforcement, season-aware ranking, automatic backups, and a no-peer watchdog shorten failed starts. Audio, quality, trust filters, connection diagnostics, release browsing, and manual selection remain available in the Advanced playback sheet.
