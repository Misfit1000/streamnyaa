...
export interface BlogPostData extends BlogPostDefinition {
  updatedAt: string;
  items: BlogMediaItem[];
}
...
function mapMedia(media: any, extra: Partial<BlogMediaItem> = {}): BlogMediaItem {
  return {
    id: media.id,
    mal_id: media.idMal || media.id,
    title: mediaTitle(media),
    description: cleanText(media.description).slice(0, 220),
...
async function fetchMediaList(sort: string, status?: string) {
  ...
  const data = await aniListRequest(query, { sort: [sort], status: status || null });
  return (data.Page.media || []).map((media: any) => mapMedia(media));
}

async function fetchSchedule(kind: 'today' | 'recent') {
  const now = new Date();
  const start = new Date(now);
  start.setHours(0, 0, 0, 0);
  const end = new Date(now);
  end.setHours(23, 59, 59, 999);
  const query = 'query BlogSchedule($start: Int, $end: Int, $now: Int) { Page(page: 1, perPage: 24) { airingSchedules(airingAt_greater: $start, airingAt_lesser: $end, sort: TIME_DESC) { episode airingAt media { ' + MEDIA_FIELDS + ' isAdult } } } }';
  const variables = kind === 'today' ? { start: Math.floor(start.getTime() / 1000), end: Math.floor(end.getTime() / 1000), now: null } : { start: null, end: Math.floor(Date.now() / 1000), now: Math.floor(Date.now() / 1000) };
...
export async function fetchBlogPost(slug: string): Promise<BlogPostData> {
  ...
  if (slug === 'trending-anime-this-week') items = await fetchMediaList('TRENDING_DESC');
  else if (slug === 'popular-anime-right-now') items = await fetchMediaList('POPULARITY_DESC');
  else if (slug === 'upcoming-anime-this-season') items = await fetchMediaList('POPULARITY_DESC', 'NOT_YET_RELEASED');
  else if (slug === 'todays-anime-release-schedule') items = await fetchSchedule('today');
  else items = await fetchSchedule('recent');
