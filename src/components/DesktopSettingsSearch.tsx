...
const sections = [
  ...
  { id:'advanced', title:'Diagnostics & tools', words:'error episode metadata connection anilist jikan mpv repair' },
];
export default function DesktopSettingsSearch() {
  const [query,setQuery] = useState('');
  const matches = sections.filter(item => `${item.title} ${item.words}`.toLowerCase().includes(query.toLowerCase().trim()));
