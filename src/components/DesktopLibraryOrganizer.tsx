...
import { renameLibraryCollection, removeLibraryCollection, removeFromLibraryCollection, createLibraryCollection, libraryOrganizationEvent, libraryStatuses, organizationIdentity, readLibraryOrganization, undoLibraryOrganization, updateLibraryOrganization, type LibraryStatus, type LibraryUndo } from '../lib/desktopLibraryOrganization';
...
export default function DesktopLibraryOrganizer({ anime }: { anime: any[] }) {
  ...
  const visible = anime.filter((item) => {
    ...
    return (status === 'All statuses' || (entry?.status || 'Bookmarked') === status)
  ...
  return <details open className="sn-glass-panel my-5 p-5">
    <summary className="cursor-pointer text-lg font-semibold">Organize library</summary>
    <p className="mt-2 text-sm text-white/60">Manage viewing status and collections on this PC. Your bookmarks and watch history stay saved.</p>
    <div className="mt-4 flex flex-wrap gap-2">
      ...
      <select aria-label="Filter collection" value={collection} onChange={(e) => {setCollection(e.target.value);setRename(e.target.value);setConfirmRemove(false);setSelected([]);}} className={selectClass}><option value="">All collections</option>{data.collections.map((value) => <option key={value}>{value}</option>)}</select>
    ...
    {collection && <div className="mt-3 flex flex-wrap items-center gap-2"><input aria-label="Rename collection" className={selectClass} maxLength={40} value={rename} onChange={e=>setRename(e.target.value)} /><button className="sn-secondary-action" onClick={()=>{try{renameLibraryCollection(collection,rename);setCollection(rename.trim());setError('');}catch(error){setError((error as Error).message);}}}>Rename</button><button className="sn-secondary-action" onClick={()=>setConfirmRemove(true)}>Delete collection</button>{confirmRemove && <span className="text-sm">Keep anime, remove collection? <button className="underline" onClick={()=>{try{removeLibraryCollection(collection);setCollection('');setConfirmRemove(false);setError('');}catch(error){setError((error as Error).message);}}}>Confirm</button> <button onClick={()=>setConfirmRemove(false)}>Cancel</button></span>}</div>}
    <div className="my-3 flex flex-wrap items-center gap-2">
      ...
      {collection && <button disabled={!selected.length} className="sn-secondary-action" onClick={()=>{try{removeFromLibraryCollection(selected,collection);setSelected([]);setError('');}catch(error){setError((error as Error).message);}}}>Remove selected from collection</button>}
    ...
    <div className="max-h-96 overflow-y-auto">
      {visible.map((item) => { const id = organizationIdentity(item); const entry = data.entries[id]; return <div key={id} className="flex items-center gap-3 border-t border-white/5 py-3">
        ...
        <select aria-label={`Viewing status for ${item.title}`} value={entry?.status || 'Bookmarked'} onChange={(e) => change([id], { status: e.target.value as LibraryStatus })} className={selectClass}>{libraryStatuses.map((value) => <option key={value}>{value}</option>)}</select>
      </div>; })}
