  ...
  return (
    <section className="group/spotlight relative mx-auto mt-4 w-full max-w-[calc(100%-24px)] overflow-hidden rounded-[28px] border border-[var(--glass-border)] bg-[#050507] shadow-[0_24px_60px_rgba(0,0,0,0.42)] md:max-w-[calc(100%-80px)]">
      <AnimatePresence mode="wait">
        ...
        >
          <div className="absolute inset-0 z-10 bg-[linear-gradient(180deg,rgba(5,5,7,0.03),rgba(5,5,7,0.1)_34%,rgba(5,5,7,0.84)_78%,rgba(5,5,7,0.98)),linear-gradient(90deg,rgba(5,5,7,0.18),rgba(5,5,7,0.04)_58%,rgba(5,5,7,0.14))] md:bg-[linear-gradient(90deg,rgba(5,5,7,0.97),rgba(5,5,7,0.82)_38%,rgba(5,5,7,0.38)_70%,rgba(5,5,7,0.78)),linear-gradient(180deg,rgba(5,5,7,0.12),rgba(5,5,7,0.95))]" />
          {heroImage ? (
            <img
              src={heroImage}
              alt={currentAnime.title}
              className={`h-full w-full object-cover ${currentAnime.banner_image ? 'object-top md:object-center' : 'object-[center_10%] md:object-[center_18%]'}`}
              style={{
      ...
      <div className="relative z-20 grid min-h-[460px] gap-8 px-5 py-5 sm:px-7 md:min-h-[560px] md:px-10 md:py-8 lg:grid-cols-[minmax(0,1fr)_280px] lg:items-center xl:grid-cols-[minmax(0,1fr)_330px]">
