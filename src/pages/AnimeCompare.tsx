...
import { ArrowRight, BarChart3, CheckCircle2, Loader2, Search, Sparkles, Star, Trophy } from 'lucide-react';
...
function StatCard({ label, left, right, winner }: { label: string; left: string; right: string; winner?: 'left' | 'right' | 'Even' }) {
  return (
    <div className="grid gap-3 rounded-2xl border border-border bg-background/45 p-4 md:grid-cols-[1fr_120px_1fr] md:items-center">
      <div className={`rounded-xl p-3 ${winner === 'left' ? 'bg-primary/10 text-primary' : 'bg-secondary/50 text-foreground'}`}>
        <p className="text-[11px] font-black uppercase tracking-wider text-muted-foreground md:hidden">{label}</p>
        <p className="text-lg font-black">{left}</p>
      </div>
      <div className="text-center">
        <p className="text-[11px] font-black uppercase tracking-wider text-muted-foreground">{label}</p>
        <p className="mt-1 text-xs font-bold text-muted-foreground">{winner === 'Even' ? 'Even' : winner === 'left' ? 'Left wins' : winner === 'right' ? 'Right wins' : 'Compare'}</p>
      </div>
      <div className={`rounded-xl p-3 text-right ${winner === 'right' ? 'bg-primary/10 text-primary' : 'bg-secondary/50 text-foreground'}`}>
        <p className="text-[11px] font-black uppercase tracking-wider text-muted-foreground md:hidden">{label}</p>
        <p className="text-lg font-black">{right}</p>
      </div>
    </div>
  );
}
        ...
        <section className="mt-10 space-y-8">
          ...
          <div className="rounded-3xl border border-[var(--glass-border)] bg-[var(--glass)] p-5 shadow-xl shadow-black/10 backdrop-blur-2xl">
            <div className="mb-5 flex items-center gap-2">
              ...
              <h2 className="text-xl font-black text-foreground">Core stats</h2>
            </div>
            <div className="space-y-3">
              <StatCard label="Score" left={left.score ? `${left.score}/10` : 'N/A'} right={right.score ? `${right.score}/10` : 'N/A'} winner={winnerLabel(left.score, right.score) as any} />
              <StatCard label="Popularity" left={formatNumber(left.popularity)} right={formatNumber(right.popularity)} winner={winnerLabel(left.popularity, right.popularity, true) as any} />
              <StatCard label="Episodes" left={left.episodes ? `${left.episodes}` : 'TBA'} right={right.episodes ? `${right.episodes}` : 'TBA'} winner={winnerLabel(left.episodes, right.episodes) as any} />
              <StatCard label="Status" left={formatStatus(left.status)} right={formatStatus(right.status)} />
              <StatCard label="Studio" left={leftStudios.slice(0, 2).join(', ') || 'TBA'} right={rightStudios.slice(0, 2).join(', ') || 'TBA'} />
            </div>
          </div>
