...
import { Calendar, ChevronRight, Clock, Download, PlayCircle, Search, TrendingUp } from 'lucide-react';
...
export default function Home() {
  ...
  const quickActions = [
    { title: 'Search anime fast', copy: 'Find title pages, genres, schedules, and episode details.', icon: <Search className="h-5 w-5" /> },
    { title: 'Compare downloads', copy: 'Jump into batch, episode, audio, and quality source results.', icon: <Download className="h-5 w-5" /> },
    { title: 'Track releases', copy: "See today's anime schedule without opening another page.", icon: <Clock className="h-5 w-5" /> },
  ];
  ...
  return (
    <div className="pb-20">
      ...
      <section className="container mx-auto mt-8 px-4 md:px-10">
        <div className="grid gap-4 md:grid-cols-3">
          {quickActions.map((item) => (
            <div key={item.title} className="rounded-3xl bg-white/[0.045] p-5 shadow-xl shadow-black/10 ring-1 ring-white/[0.06] backdrop-blur-2xl">
              <div className="mb-4 flex h-11 w-11 items-center justify-center rounded-2xl bg-primary/15 text-primary">{item.icon}</div>
      ...
      <main className="container mx-auto mt-14 grid grid-cols-1 gap-12 px-4 md:px-10 lg:grid-cols-[minmax(0,1fr)_340px]">
        <div className="space-y-16">
          ...
          <section>
            ...
            <div className="rounded-[32px] bg-white/[0.035] p-4 shadow-xl shadow-black/10 ring-1 ring-white/[0.055] backdrop-blur-2xl md:p-5">
              <div className="grid grid-cols-2 gap-x-4 gap-y-8 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5">
