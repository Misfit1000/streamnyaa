...
describe('native recovery relay handler', () => {
  it('rejects unknown platforms and actions', () => {
    ...
    handler({ method: 'GET', query: { route: 'native-callback', platform: 'web', action: 'recovery' } }, invalidPlatform.response);
    expect(invalidPlatform.response.statusCode).toBe(400);

    const invalidAction = responseHarness();
    handler({ method: 'GET', query: { route: 'native-callback', platform: 'desktop', action: 'login' } }, invalidAction.response);
    expect(invalidAction.response.statusCode).toBe(400);

    const unknownParameter = responseHarness();
    handler({ method: 'GET', query: { route: 'native-callback', platform: 'desktop', action: 'recovery', next: 'https://evil.example' } }, unknownParameter.response);
  ...
  it('returns a non-cacheable credential-filtering handoff', () => {
    ...
    handler({ method: 'GET', query: { route: 'native-callback', platform: 'desktop', action: 'recovery' } }, response);
