...
try {
  ...
  if ($LASTEXITCODE -ne 0 -or $output -notmatch 'player-state regression passed: 34 assertions' -or $output -match 'Lua error:|regression FAILED:') {
    Write-Host $output
    throw 'Native player-state regression failed.'
  }
  Write-Host 'Native player-state regression passed (34 behavioral assertions).'
}
finally {
  Pop-Location
}
