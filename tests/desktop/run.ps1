...
Assert-Match $tauriConfig '"version"\s*:\s*"0\.1\.6"' 'Desktop release configuration should target version 0.1.6.'
Assert-Match $cargoManifest 'version\s*=\s*"0\.1\.6"' 'Rust package version should match the desktop release version.'
Assert-Match $releasePreparer "releaseRevision\s*=\s*'desktop-loading-presentation-1'" 'The 0.1.6 release manifest must identify the loading presentation pass.'
...
Assert-Match $desktopShell 'HOVER_PRELOAD_DELAY_MS\s*=\s*110' 'Desktop hover preloading should wait for navigation intent instead of loading on incidental pointer passes.'
