...
Assert-Match $tauriConfig '"version"\s*:\s*"0\.1\.5"' 'Desktop release configuration should target version 0.1.5.'
Assert-Match $cargoManifest 'version\s*=\s*"0\.1\.5"' 'Rust package version should match the desktop release version.'
Assert-Match $releasePreparer "releaseRevision\s*=\s*'desktop-upward-optimization-1'" 'The 0.1.5 release manifest must identify the upward optimization pass.'
...
Assert-Match $mainDesktop 'sn-boot--leaving' 'Desktop entry should dismiss the launch screen after the first React paint.'
