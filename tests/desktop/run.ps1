...
Assert-Match $desktopBridge 'coveragePercent\(coverage,durationSeconds\)' 'Torrent completion must not be stored as episode watch completion.'
...
Assert-Match $watchPage 'sourcesBusy\s*=\s*sourceSearchInProgress\s*&&\s*!playableSources\.some' 'Playback may proceed during background search only when a playable source exists.'
...
Assert-Match $desktopExplore "season: mode === 'seasonal' \? season : undefined" 'Desktop Explore should send its selected season through the paginated catalog request.'
...
Assert-Match $desktopSettings 'Export personal backup' 'Desktop Settings should expose safe settings export.'
Assert-Match $desktopSettings 'Restore personal backup' 'Desktop Settings should expose safe settings import.'
...
Assert-Match $desktopShell 'navigate\(desktopSearchPath\(query, location.pathname, location.search\)\)' 'Submitting desktop search must use the shared, filter-preserving Explore route.'
Assert-Match $desktopExplore 'allowFallback: pageParam.page === 1' 'Explore should choose its fallback on the first page and preserve provider pagination.'
...
Assert-Match $desktopQueryClient 'refetchOnReconnect:\s*desktop' 'Active desktop screens must refresh after connectivity returns.'
...
Assert-Match $playerSkin '(?s)function close_menu_or_overlay\(\).*?elseif mp\.get_property_native\("fullscreen", state\.fullscreen\) then.*?safe_set_property\("fullscreen", "no"\)' 'Escape should exit native fullscreen after closing player menus.'
...
Assert-Match $playerSkin '(?s)function begin_first_video_frame_handoff\(\).*?clear_cover_overlay\(\)' 'Decoded video handoff must remove artwork without a timed blend over video.'
...
Assert-Match $playerSkin 'Measuring buffer' 'Unavailable buffer telemetry must not fabricate a percentage.'
Assert-Match $playerSkin 'MAX_CONTINUOUS_BUFFERING_SECONDS\s*=\s*60' 'Trickle data must not indefinitely postpone recovery.'
Assert-Match $playerSkin 'function check_recovery_deadline' 'Recovery deadlines must survive native reload events.'
Assert-NotMatch $playerSkin 'function stall_watchdog_allowed\(\)\r?\n[^\r\n]*ui\.settings_open' 'Player settings must never suspend automatic recovery.'
Assert-Match $tauriMain 'deliberately_paused = paused.unwrap_or\(false\) && !paused_for_cache' 'Cache pauses must not be reported as intentional pauses.'
Assert-NotMatch $playerSkin 'create_osd_overlay, "bgra"' 'Bitmap art must use the supported overlay-add API.'
...
Assert-Match $playerSkin 'string\.format\("%.1fs buffered"' 'Native startup must show measured playable seconds without contradicting MPV cache-fill progress.'
...
Assert-Match $desktopHome 'const detailsPath = source.animeId \? desktopWatchPath' 'Continue Watching details should open the anime Watch page.'
...
Assert-Match $desktopExplore "queryKey: \['desktop-explore-v3', catalogCacheKey\]" 'Desktop Explore should cache each primary catalog selection independently.'
...
Assert-Match $tauriMain 'portrait_player_art_supplied_in_both_fields_remains_available' 'Poster-only artwork must remain available even when repeated as a banner.'
Assert-Match $tauriMain 'player_art_candidates_prefer_banners_and_retain_poster_fallback' 'Loading artwork must prefer banners and retain a poster fallback.'
...
Assert-Match $playerSkin 'local base_offset = ui\.visible and 180 or 140' 'Manual intro and outro actions should remain raised above the player edge and controls.'
